...
export async function GET(request: Request) {
  ...
  if (!type || !['weather', 'forecast', 'air_pollution', 'air_pollution_forecast'].includes(type)) {
    return NextResponse.json({ error: 'Invalid or missing type parameter' }, { status: 400 });
  }

  if (!apiKey) {
    console.error('weather/route.ts: OPENWEATHERMAP_API_KEY not configured');
  ...
  try {
    ...
    switch (type) {
      ...
      case 'forecast':
        ...
        break;
      case 'air_pollution':
        url = `https://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${apiKey}&lang=en`;
        break;
    ...
    if (!response.ok) {
      ...
      console.error(`weather/route.ts: OpenWeatherMap API failed for ${type}:`, response.status, errorText);
    ...
    console.error(`weather/route.ts: Error fetching ${type} data:`, error);
